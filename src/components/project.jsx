const projects = [
  {
    ...
    period: 'Feb – Apr 2023',
    description: 'Redesigned the mobile version of Sriwijaya University\'s academic information system (SIMAK) using the Design Thinking framework. Conducted comprehensive user research, usability testing, and delivered an improved UI prototype focusing on clarity, accessibility, and modern mobile experience.',
  ...
  },
  {
    ...
    period: '2025',
    description: 'Full frontend portfolio project for a digital pulsa counter platform. Built with React + Vite featuring a modern glassmorphism UI with dark theme, animated backgrounds, and complete transaction flow — Pulsa, Paket Data, PLN Token, Top Up, and transaction history.',
  ...
  },
  {
    ...
    subtitle: 'Frontend Development · React',
    period: 'Feb – Apr 2022',
    description: 'Sistem Manajemen Aset Teknologi Informasi untuk Diskominfo Kota Palembang. Mengelola inventaris perangkat IT, peminjaman aset, maintenance & servis, manajemen user, serta laporan analisis berbasis data. Dibangun dengan React + Vite dengan tampilan clean government-modern.',
    ...
    <section id="projects" style={{ background:'rgba(255,255,255,0.015)', borderTop:'1px solid rgba(255,255,255,0.05)', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
      <div className="section">
        ...
        {projects.filter(p => p.featured).map(p => (
          <div key={p.id} className="fade-up d1" style={{ marginBottom:20 }}>
            <div style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.transform='translateY(-4px)'; e.currentTarget.style.boxShadow=`0 24px 60px ${p.color}20` }}
              onMouseLeave={e => { e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='none' }}
            >
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
                  <div style={{ width:44, height:44, borderRadius:12, background:`${p.color}20`, border:`1px solid ${p.color}40`, display:'flex', alignItems:'center', justifyContent:'center', color:p.color }}>
                    {p.icon}
                  </div>
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:'2px 10px', borderRadius:99, background:`${p.color}20`, color:p.color, border:`1px solid ${p.color}30` }}>FEATURED</span>
                      <span style={{ fontSize:11, color:'rgba(255,255,255,0.3)', fontWeight:500 }}>{p.period}</span>
                    </div>
                    <h3 style={{ fontSize:22, fontWeight:800, color:'#fff', marginTop:4, letterSpacing:'-0.3px' }}>{p.title}</h3>
                  </div>
                </div>
                <p style={{ fontSize:14, color:'rgba(255,255,255,0.5)', lineHeight:1.75, maxWidth:580, marginBottom:20 }}>{p.description}</p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:24 }}>
                  {p.tags.map(t => <span key={t} className="tag tag-purple">{t}</span>)}
                </div>
          ...
          {projects.filter(p => !p.featured).map((p, i) => (
            ...
            >
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:16 }}>
                {p.tags.slice(0,4).map(t => <span key={t} className="tag" style={{ fontSize:'10px', background:`${p.color}12`, color:p.color, borderColor:`${p.color}30` }}>{t}</span>)}
              </div>
